...
### Tool Architecture
The project adopts a modular architecture with two main toolsets:

- **CMS Toolkit** (Observability 2.0, Recommended): Contains 8 modern tool modules including entities, metrics, traces, events, topologies, diagnosis, drilldown, and workspace
- **IaaS Toolkit** (V1 Compatibility): Legacy SLS, ARMS, CMS tools that maintain backward compatibility

The system will automatically register available toolkits based on installed optional dependencies.
...
- `--transport` Specify the transport method, options are `stdio`, `sse`, or `streamable-http`, default is `streamable-http`
...
- `--sls-endpoints` Override SLS endpoint mapping with `REGION=HOST` pairs separated by comma/space, e.g. `--sls-endpoints "cn-shanghai=cn-hangzhou.log.aliyuncs.com"`
- `--cms-endpoints` Override CMS endpoint mapping, same format as above, e.g. `--cms-endpoints "cn-shanghai=cms.internal.aliyuncs.com"`
...
- `--transport-port` Specify transport port, default is `8080`, only effective when `--transport` is `sse` or `streamable-http`
