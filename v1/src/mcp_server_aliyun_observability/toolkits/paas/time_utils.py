...
def _parse_datetime_string(expr: str, now: datetime) -> int:
    ...
    raise ValueError(
        f"无法解析日期时间字符串: {expr}，支持格式: YYYY-MM-DD, YYYY-MM-DD HH:MM, YYYY-MM-DD HH:MM:SS"
    )
...
def compute_time_range(
    expr: Optional[str], now: Optional[datetime] = None
) -> Tuple[int, int]:
...
def format_timestamp(timestamp: int, tz: Optional[timezone] = None) -> str:
...
class TimeRangeParser:
    ...
    def parse_time_expression(
        time_expr: Union[str, int], now: Optional[int] = None
    ) -> int:
        """解析时间表达式为Unix时间戳（秒）
    ...
    def parse_time_range(
        from_time: Union[str, int], to_time: Union[str, int]
    ) -> Tuple[int, int]:
        """解析时间范围
