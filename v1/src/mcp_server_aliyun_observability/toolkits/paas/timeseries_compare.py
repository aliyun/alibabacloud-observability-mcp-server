...
def analyze_trend(
    current_stats: TimeSeriesStats,
    compare_stats: Optional[TimeSeriesStats]
) -> DiffDetails:
    """分析变化趋势
...
def parse_time_series_data(
    ...
    for row in data:
        ...
        key = TimeSeriesKey(
            entity_id=entity_id,
            labels=labels,
            metric=str(row.get("metric", "")) if key_type == KeyType.GOLDEN_METRICS else "",
            metric_set_id=str(row.get("metric_set_id", "")) if key_type == KeyType.GOLDEN_METRICS else ""
        )
...
def sort_by_diff_score(results: List[TimeSeriesCompareResult]) -> List[TimeSeriesCompareResult]:
    """按差异评分降序排序
...
def compare_output_to_dict(output: CompareOutput) -> Dict[str, Any]:
    ...
    return {
        ...
        "compare_time_range": time_range_to_dict(output.compare_time_range) if output.compare_time_range else None,
