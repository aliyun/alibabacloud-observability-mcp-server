...
class EntityFuzzySelector(BaseModel):
    ...
    limit: int = Field(default=100, description="返回结果数量限制，默认100")
...
class TimeRange(BaseModel):
    ...
    end_time: str = Field(
        default="now()", description="结束时间，支持相对时间表达式如 now() 或绝对时间戳"
    )
...
class MetricQuery(BaseModel):
    ...
    aggregation: Optional[str] = Field(
        None, description="聚合方式（如 avg, sum, max, min, count）"
    )
    interval: Optional[str] = Field("1m", description="数据点间隔，默认1分钟")


class TraceFilter(BaseModel):
    """链路过滤条件"""

    error: Optional[bool] = Field(None, description="是否只查询错误链路")
    min_duration: Optional[int] = Field(None, description="最小耗时（毫秒）")
    max_duration: Optional[int] = Field(None, description="最大耗时（毫秒）")
    status_codes: Optional[List[str]] = Field(None, description="HTTP状态码列表")


class EventFilter(BaseModel):
    """事件过滤条件"""

    event_type: Optional[str] = Field(None, description="事件类型（如 change, alert）")
    severity: Optional[str] = Field(
        None, description="严重程度（如 critical, warning, info）"
    )
