...
class CMSClientWrapper:
    ...
    def with_legacy_region(
        self, region: str, endpoint: Optional[str] = None
    ) -> CmsLegacyClient:
        config = _create_unified_config(self.credential, use_signature_v3=True)
...
def text_to_sql_old(
    ...
    try:
        ...
        params: dict[str, Any] = {
            ...
            "external_knowledge_uri": (
                knowledge_config["uri"] if knowledge_config else ""
            ),
            "external_knowledge_key": (
                knowledge_config["key"] if knowledge_config else ""
            ),
        }
    ...
    try:
        ...
        user_context = json.dumps(
            [
                ...
                {
                    "type": "spl_generation",
                    "data": {"data": data_sample if data_sample else "[]"},
                },
            ]
        )
    ...
    try:
        ...
        messages = [
            cms_model.CreateChatRequestMessages(
                role="user",
                contents=[
                    cms_model.CreateChatRequestMessagesContents(type="text", value=text)
                ],
            )
        ]
...
def text_to_sql(
    ...
    try:
        ...
        collected_sql = None
        collected_explanation = []
        trace_id = None  # noqa: F841  # 保留：SSE 流式响应中收集的 trace_id，后续可能用于排障/日志关联
        ...
        for response in cms_client.create_chat_with_sse(chat_request, {}, runtime):
            ...
            if (
                ...
                trace_id = response.body.trace_id  # noqa: F841
