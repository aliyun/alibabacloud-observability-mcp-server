...
@click.option(
    "--region", "-r", default="cn-hangzhou", help="Default Alibaba Cloud region"
)
@click.option(
    "--config-file",
    "-c",
...
def show_detailed_tools(scope):
    ...
    console.print(
        f"[yellow]ℹ[/yellow] {info['tools']} tools available with prefix '{info['prefix']}'"
    )
...
def show_tools_simple(scope):
    ...
    if scope == "all":
        for key, info in TOOLKIT_CATEGORIES.items():
            if key != "all":
                console.print(
                    f"{info['name']}: {info['tools']} tools ({info['prefix']})"
                )
    else:
        info = TOOLKIT_CATEGORIES[scope]
...
def config_init(force):
    ...
    if config_file.exists() and not force:
        console.print(
            f"[yellow]⚠[/yellow] Configuration already exists at {config_file}"
        )
